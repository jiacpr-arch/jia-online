                ...
                <div style={{ marginBottom: 20 }}>
                  <div style={{ borderRadius: 16, overflow: "hidden" }}>
                    <iframe
                      ...
                      src={"https://www.youtube.com/embed/" + mod.dur + "?rel=0"}
                      title={mod.title}
                  ...
                  <button onClick={() => setWatched(true)} style={{
                    width: "100%", marginTop: 12, background: B.green, color: B.white,
                    border: "none", borderRadius: 10, padding: "14px", fontSize: 14, fontWeight: 600, cursor: "pointer",
                  }}>
                    ดูวิดีโอจบแล้ว ✓ → ทำแบบทดสอบ
                  </button>
                </div>
              ) : (
                <div style={{
                  background: `${B.green}15`, borderRadius: 16, padding: 20,
                  textAlign: "center", marginBottom: 20, border: `1px solid ${B.green}30`,
                }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: B.dkGray, marginTop: 2 }}>
                  {dn ? `✓ ผ่านแล้ว ${progress.scores[m.id] ? `(${progress.scores[m.id]}%)` : ""}` :
                    m.dur ? `วิดีโอ + ${m.quiz.length} คำถาม • ต้องได้ 80%` : `${m.quiz.length} คำถาม • ต้องได้ 80%`}
                </div>
              </div>
