...
const COURSE = {
  ...
  modules: [
    { id: 1, title: "บทที่ 1: CPR ผู้ใหญ่", short: "CPR ผู้ใหญ่", desc: "เทคนิคกดหน้าอก เปิดทางเดินหายใจ ช่วยหายใจสำหรับผู้ใหญ่", vid: "dtIswl6Od2I", quiz: [
    ...
    { id: 2, title: "บทที่ 2: CPR ทารก", short: "CPR ทารก", desc: "เทคนิค CPR สำหรับทารก ความแตกต่างจากผู้ใหญ่", vid: "0nL-4GvDxVM", quiz: [
    ...
    { id: 3, title: "บทที่ 3: สิ่งอุดกั้นทางเดินหายใจ ผู้ใหญ่", short: "Choking ผู้ใหญ่", desc: "วิธีช่วยเหลือผู้ใหญ่ที่มีสิ่งอุดกั้นทางเดินหายใจ (Heimlich Maneuver)", vid: "t3WnWOOVh2c", quiz: [
    ...
    { id: 4, title: "บทที่ 4: สิ่งอุดกั้นทางเดินหายใจ ทารก", short: "Choking ทารก", desc: "วิธีช่วยเหลือทารกที่มีสิ่งอุดกั้นทางเดินหายใจ", vid: "uovi2Z1RAuI", quiz: [
    ...
    { id: 5, title: "บทที่ 5: Megacode — CPR & AED ผู้ใหญ่", short: "CPR & AED ผู้ใหญ่ (Megacode)", desc: "ฝึกปฏิบัติ CPR ร่วมกับเครื่อง AED แบบครบขั้นตอนสำหรับผู้ใหญ่", vid: "r5CcjzGhNL8", quiz: [
    ...
    { id: 6, title: "บทที่ 6: Megacode — CPR & AED ทารก", short: "CPR & AED ทารก (Megacode)", desc: "ฝึกปฏิบัติ CPR ร่วมกับเครื่อง AED แบบครบขั้นตอนสำหรับทารก", vid: "0nL-4GvDxVM", quiz: [
    ...
    { id: 7, title: "แบบทดสอบสุดท้าย", short: "Final Exam", desc: "ทดสอบความรู้ทั้งหมด 6 บท ต้องได้ 80% ขึ้นไปจึงผ่าน", vid: null, quiz: [
...
function Steps({ current }) {
  ...
  return (<div style={{ display: "flex", gap: 6, marginBottom: 28, justifyContent: "center", flexWrap: "wrap" }}>{steps.map((s, i) => (<div key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 26, height: 26, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, background: i < current ? B.green : i === current ? B.red : B.ltGray, color: i <= current ? B.white : B.dkGray }}>{i < current ? <I name="check" size={13} color={B.white}/> : i + 1}</div><span style={{ fontSize: 11, fontWeight: i === current ? 600 : 400, color: i <= current ? B.black : B.dkGray }}>{s}</span>{i < steps.length - 1 && <div style={{ width: 16, height: 1, background: B.ltGray }}/>}</div>))}</div>);
}
  ...
  return (
    <div style={css.page}>
      <div style={{ background: `linear-gradient(135deg, ${B.red} 0%, ${B.dkRed} 100%)`, color: B.white, padding: "52px 24px 56px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: -60, right: -60, width: 240, height: 240, borderRadius: "50%", background: "rgba(255,255,255,.06)" }}/><div style={{ position: "absolute", bottom: -40, left: -40, width: 180, height: 180, borderRadius: "50%", background: "rgba(255,255,255,.04)" }}/>
        <div style={{ position: "relative", zIndex: 1, maxWidth: 480, margin: "0 auto", opacity: anim ? 1 : 0, transform: anim ? "translateY(0)" : "translateY(20px)", transition: "all .6s ease" }}>
          ...
          <p style={{ fontSize: 14, opacity: .9, lineHeight: 1.7, marginBottom: 28 }}>เรียนรู้การช่วยชีวิตขั้นพื้นฐาน มาตรฐานสากล<br/>ดูวิดีโอ • ทำแบบทดสอบ • รับใบประกาศนียบัตร</p>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 14, background: "rgba(255,255,255,.15)", borderRadius: 16, padding: "14px 28px", backdropFilter: "blur(10px)", marginBottom: 28 }}>
            {FREE_LAUNCH ? (<><span style={{ fontSize: 44, fontWeight: 800, lineHeight: 1 }}>ฟรี!</span><div style={{ textAlign: "left", fontSize: 12, lineHeight: 1.4 }}><div style={{ fontWeight: 600, textDecoration: "line-through", opacity: .7 }}>ปกติ ฿100</div><div style={{ opacity: .85 }}>+ คูปองส่วนลด ฿100 ตอนมาเรียน on-site</div></div></>) : (<><span style={{ fontSize: 44, fontWeight: 800, lineHeight: 1 }}>฿100</span><div style={{ textAlign: "left", fontSize: 12, lineHeight: 1.4 }}><div style={{ fontWeight: 600 }}>เท่านั้น!</div><div style={{ opacity: .85 }}>เอาเป็นส่วนลดตอนมาเรียน on-site</div></div></>)}
          </div>
          <div><button onClick={() => go("register")} style={{ ...css.btn(B.white, B.red), padding: "16px 52px", fontSize: 16, boxShadow: "0 4px 24px rgba(0,0,0,.2)" }}>{FREE_LAUNCH ? "ลงทะเบียนเรียนฟรี →" : "สมัครเรียนเลย →"}</button></div>
        </div>
      </div>
      <div style={{ ...css.wrap, paddingTop: 36, paddingBottom: 24 }}>
        ...
        {COURSE.modules.slice(0, 6).map((m, i) => (<div key={i} style={{ display: "flex", gap: 14, alignItems: "flex-start", marginBottom: 12, background: B.white, borderRadius: 14, padding: "14px 16px", boxShadow: "0 1px 6px rgba(0,0,0,.04)" }}><div style={{ minWidth: 38, height: 38, borderRadius: 10, background: `${B.red}12`, display: "flex", alignItems: "center", justifyContent: "center", color: B.red, fontWeight: 800, fontSize: 15 }}>{String(i + 1).padStart(2, "0")}</div><div><div style={{ fontWeight: 600, fontSize: 14, marginBottom: 3 }}>{m.short}</div><div style={{ fontSize: 12, color: B.dkGray, lineHeight: 1.5 }}>{m.desc}</div></div></div>))}
        <div style={{ background: `${B.gold}18`, borderRadius: 14, padding: 16, textAlign: "center", border: `1px solid ${B.gold}35`, marginTop: 4 }}><I name="cert" size={26} color={B.gold}/><div style={{ fontWeight: 600, fontSize: 14, marginTop: 6 }}>+ แบบทดสอบสุดท้าย & ใบประกาศนียบัตร</div><div style={{ fontSize: 12, color: B.dkGray, marginTop: 3 }}>ผ่าน 80% ขึ้นไป ได้รับใบรับรองมาตรฐานสากล</div></div>
      </div>
      <div style={{ ...css.wrap, paddingBottom: 24 }}>
        <div style={{ background: B.white, borderRadius: 16, padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,.06)", border: `1px solid ${B.red}12` }}>
          <div style={{ textAlign: "center", marginBottom: 16 }}><div style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 44, height: 44, borderRadius: 12, background: `${B.red}10`, marginBottom: 8 }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={B.red} strokeWidth="2"><path d="M2 3h6a4 4 0 014 4v14a3 3 0 00-3-3H2z"/><path d="M22 3h-6a4 4 0 00-4 4v14a3 3 0 013-3h7z"/></svg></div><h3 style={{ fontSize: 17, fontWeight: 700, margin: 0 }}>ทำไมต้องเรียน CPR?</h3><p style={{ fontSize: 12, color: B.dkGray, marginTop: 4, marginBottom: 0 }}>ข้อมูลจากงานวิจัย</p></div>
          <div style={{ background: `${B.red}06`, borderRadius: 12, padding: 14, marginBottom: 14, borderLeft: `4px solid ${B.red}` }}><div style={{ fontSize: 28, fontWeight: 800, color: B.red, lineHeight: 1 }}>10%</div><div style={{ fontSize: 13, marginTop: 4, lineHeight: 1.5 }}>ทุก 1 นาทีที่ไม่ได้ทำ CPR<br/>โอกาสรอดชีวิตลดลง 10%</div></div>
          <div style={{ background: `${B.gold}10`, borderRadius: 12, padding: 14, marginBottom: 14, borderLeft: `4px solid ${B.gold}` }}><div style={{ fontSize: 28, fontWeight: 800, color: B.gold, lineHeight: 1 }}>3-6 เดือน</div><div style={{ fontSize: 13, marginTop: 4, lineHeight: 1.5 }}>งานวิจัยพบว่าทักษะ CPR เสื่อมลง<br/>ภายใน 3-6 เดือนหลังอบรม</div></div>
          <div style={{ background: `${B.green}10`, borderRadius: 12, padding: 14, marginBottom: 14, borderLeft: `4px solid ${B.green}` }}><div style={{ fontSize: 28, fontWeight: 800, color: B.green, lineHeight: 1 }}>58%</div><div style={{ fontSize: 13, marginTop: 4, lineHeight: 1.5 }}>ผู้ที่ทบทวนทุกเดือน ทำ CPR ได้ระดับ "ดีเยี่ยม"<br/>เทียบกับ 15% ที่ทบทวนปีละครั้ง</div></div>
          <div style={{ background: B.cream, borderRadius: 10, padding: 12, textAlign: "center", fontSize: 13, lineHeight: 1.6, fontWeight: 500 }}>คอร์สออนไลน์นี้ช่วยให้คุณทบทวนความรู้ CPR<br/>ได้ทุกเมื่อ <strong style={{ color: B.red }}>ไม่ต้องรอปีละครั้ง</strong></div>
        </div>
      </div>
      <div style={{ ...css.wrap, paddingBottom: 100 }}><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>{[{ icon: "play", l: "6 วิดีโอ", s: "เรียนได้ทุกที่ทุกเวลา" },{ icon: "book", l: "Quiz ทุกบท", s: "ทดสอบความเข้าใจ" },{ icon: "cert", l: "ใบประกาศฯ", s: "มาตรฐานสากล" },{ icon: "heart", l: FREE_LAUNCH ? "คูปอง ฿100" : "ส่วนลด ฿100", s: "ใช้ตอนเรียน on-site" }].map((f, i) => (<div key={i} style={{ background: B.white, borderRadius: 14, padding: 16, textAlign: "center", boxShadow: "0 1px 6px rgba(0,0,0,.04)" }}><I name={f.icon} size={22} color={B.red}/><div style={{ fontWeight: 600, fontSize: 13, marginTop: 6 }}>{f.l}</div><div style={{ fontSize: 11, color: B.dkGray, marginTop: 2 }}>{f.s}</div></div>))}</div></div>
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: B.white, padding: "14px 20px", boxShadow: "0 -4px 24px rgba(0,0,0,.08)", zIndex: 100 }}><div style={{ maxWidth: 480, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between" }}><div><div style={{ fontSize: 11, color: B.dkGray }}>{FREE_LAUNCH ? "ช่วง Launch พิเศษ" : "ราคาคอร์สออนไลน์"}</div><div style={{ fontSize: 22, fontWeight: 800, color: B.red }}>{FREE_LAUNCH ? "ฟรี!" : "฿100"}</div></div><button onClick={() => go("register")} style={css.btn(B.red, B.white)}>{FREE_LAUNCH ? "เรียนฟรี" : "สมัครเรียน"}</button></div></div>
    </div>
  );
}
...
function Register({ go, setUser }) {
  ...
  const submit = () => {
    const e = {}; if (!f.name.trim()) e.name = "กรุณากรอกชื่อ-นามสกุล"; if (!f.phone.trim() || f.phone.replace(/\D/g, "").length < 9) e.phone = "กรุณากรอกเบอร์โทรที่ถูกต้อง"; if (Object.keys(e).length) return setErr(e);
    setUser(f); save("user", f); sendToSheet({ action: "register", name: f.name, phone: f.phone, email: f.email });
  ...
  return (<div style={css.page}><div style={css.header(B.red)}><button onClick={() => go("landing")} style={{ background: "none", border: "none", padding: 0, cursor: "pointer" }}><I name="back" size={24} color={B.white}/></button><div><div style={{ fontSize: 16, fontWeight: 700 }}>สมัครเรียน</div></div></div><div style={{ ...css.wrap, paddingTop: 24, paddingBottom: 40 }}><Steps current={0}/><div style={css.card}><h3 style={{ fontSize: 17, fontWeight: 700, marginTop: 0, marginBottom: 18 }}>ข้อมูลผู้เรียน</h3>{field("name", "ชื่อ-นามสกุล *", "เช่น สมชาย ใจดี")}{field("phone", "เบอร์โทรศัพท์ *", "เช่น 081-234-5678", "tel")}{field("email", "อีเมล (ไม่บังคับ)", "เช่น name@email.com", "email")}</div><div style={{ ...css.card, marginTop: 14, padding: 18 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: 14, fontWeight: 600 }}>คอร์ส CPR & AED ออนไลน์</span><span style={{ fontSize: 18, fontWeight: 800, color: FREE_LAUNCH ? B.green : B.red }}>{FREE_LAUNCH ? "ฟรี!" : "฿100"}</span></div><div style={{ fontSize: 12, color: B.dkGray, marginTop: 6, lineHeight: 1.5 }}>6 บทเรียน + Final Exam • ใบประกาศนียบัตร • คูปองส่วนลด ฿100 เมื่อมาเรียน on-site</div></div><button onClick={submit} style={{ ...css.btn(B.red, B.white, true), marginTop: 20, boxShadow: `0 4px 16px ${B.red}40` }}>{FREE_LAUNCH ? "ลงทะเบียน → เข้าเรียนเลย" : "ถัดไป → ชำระเงิน"}</button></div></div>);
}

// ==================== PAYMENT ====================
function Payment({ go }) {
  const [slipSent, setSlipSent] = useState(false); const [approved, setApproved] = useState(false);
  return (<div style={css.page}><div style={css.header(B.red)}><button onClick={() => go("register")} style={{ background: "none", border: "none", padding: 0, cursor: "pointer" }}><I name="back" size={24} color={B.white}/></button><div><div style={{ fontSize: 16, fontWeight: 700 }}>ชำระเงิน</div></div></div><div style={{ ...css.wrap, paddingTop: 24, paddingBottom: 40 }}><Steps current={1}/>{!approved ? (<><div style={{ ...css.card, textAlign: "center" }}><div style={{ fontSize: 14, fontWeight: 600, marginBottom: 16 }}>สแกน QR Code เพื่อโอนเงิน</div><div style={{ width: 200, height: 200, background: B.gray, borderRadius: 14, margin: "0 auto 16px", display: "flex", alignItems: "center", justifyContent: "center", border: `2px dashed ${B.ltGray}` }}><div style={{ textAlign: "center" }}><I name="qr" size={48} color={B.dkGray}/><div style={{ fontSize: 11, color: B.dkGray, marginTop: 8 }}>QR PromptPay</div></div></div><div style={{ background: `${B.red}08`, borderRadius: 12, padding: 14 }}><div style={{ fontSize: 12, color: B.dkGray }}>ยอดชำระ</div><div style={{ fontSize: 30, fontWeight: 800, color: B.red }}>฿100.00</div></div></div><a href="https://lin.ee/jiacpr" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 14, background: "#06C755", borderRadius: 16, padding: "16px 20px", color: B.white, textDecoration: "none" }}><I name="line" size={36} color={B.white}/><div><div style={{ fontWeight: 700, fontSize: 14 }}>ส่งสลิปทาง LINE OA</div><div style={{ fontSize: 20, fontWeight: 800 }}>@jiacpr</div></div></a><button onClick={() => setSlipSent(true)} style={{ ...css.btn(slipSent ? B.green : B.red, B.white, true), marginTop: 18 }}>{slipSent ? "✓ บันทึกแล้ว" : "ส่งสลิปแล้ว ✓"}</button>{slipSent && <div style={{ background: `${B.green}12`, borderRadius: 14, padding: 18, marginTop: 14, textAlign: "center" }}><div style={{ fontSize: 14, fontWeight: 600, color: B.green }}>รอแอดมินตรวจสอบ</div><button onClick={() => { setApproved(true); save("enrolled", true); }} style={{ ...css.btn(B.green, B.white), marginTop: 12, padding: "10px 20px", fontSize: 13 }}>(Demo) อนุมัติ →</button></div>}</>) : (<div style={{ textAlign: "center", padding: "40px 0" }}><div style={{ width: 80, height: 80, borderRadius: "50%", background: `${B.green}18`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}><I name="check" size={40} color={B.green}/></div><h3 style={{ fontSize: 20, fontWeight: 700, margin: "0 0 8px" }}>ชำระเงินสำเร็จ!</h3><button onClick={() => go("course")} style={{ ...css.btn(B.red, B.white), fontSize: 16, padding: "16px 48px" }}>เข้าเรียนเลย →</button></div>)}</div></div>);
}

// ==================== COURSE ====================
function Course({ go, progress, setProgress, user }) {
  const [active, setActive] = useState(null); const [quiz, setQuiz] = useState(false); const [ans, setAns] = useState({}); const [result, setResult] = useState(null); const [watched, setWatched] = useState(false); const [reviewMode, setReviewMode] = useState(false);
  ...
  const submitQuiz = () => {
    ...
    if (passed && !progress.done.includes(active)) { const np = { ...progress, done: [...progress.done, active], scores: { ...progress.scores, [active]: score } }; setProgress(np); save("progress", np);
      const isFinalExam = !mod.vid && mod.id === COURSE.modules[COURSE.modules.length - 1].id;
      if (isFinalExam && user) { const coupon = genCoupon(); save("coupon", coupon); sendToSheet({ action: "complete", name: user.name, phone: user.phone, score, coupon }); }
    }
  };
  const resetLesson = () => { setActive(null); setQuiz(false); setAns({}); setResult(null); setWatched(false); setReviewMode(false); };
  if (active) {
    ...
    return (<div style={css.page}><div style={css.header(B.black)}><button onClick={resetLesson} style={{ background: "none", border: "none", padding: 0, cursor: "pointer" }}><I name="back" size={24} color={B.white}/></button><div style={{ flex: 1, fontSize: 14, fontWeight: 700, color: B.white }}>{mod.title}</div></div>
      <div style={{ ...css.wrap, paddingTop: 24, paddingBottom: 40 }}>
        {(!quiz && !isFinal) || reviewMode ? (<>
          <div style={{ borderRadius: 16, overflow: "hidden", marginBottom: 20 }}><iframe width="100%" style={{ aspectRatio: "16/9", border: "none", display: "block" }} src={"https://www.youtube.com/embed/" + mod.vid + "?rel=0"} title={mod.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/></div>
          <p style={{ fontSize: 14, color: B.dkGray, lineHeight: 1.6, marginBottom: 20 }}>{mod.desc}</p>
          ...
          <div style={css.card}><h3 style={{ fontSize: 16, fontWeight: 700, marginTop: 0, marginBottom: 4 }}>{isFinal ? "แบบทดสอบสุดท้าย" : "แบบทดสอบท้ายบท"}</h3><p style={{ fontSize: 12, color: B.dkGray, margin: "0 0 20px" }}>ต้องได้ 80% ขึ้นไป ({Math.ceil(mod.quiz.length * 0.8)}/{mod.quiz.length} ข้อ)</p>
            ...
            : <div style={{ textAlign: "center" }}><div style={{ background: result.passed ? `${B.green}12` : `${B.red}08`, borderRadius: 12, padding: 20, marginBottom: 16 }}><div style={{ fontSize: 40, fontWeight: 800, color: result.passed ? B.green : B.red }}>{result.score}%</div><div style={{ fontSize: 14, fontWeight: 600, color: result.passed ? B.green : B.red }}>{result.passed ? "ผ่าน!" : "ไม่ผ่าน"}</div><div style={{ fontSize: 12, color: B.dkGray, marginTop: 4 }}>ตอบถูก {result.correct}/{result.total} ข้อ</div></div>{result.passed ? <button onClick={() => { resetLesson(); if (isFinal) go("certificate"); }} style={css.btn(B.green, B.white)}>{isFinal ? "รับใบประกาศนียบัตร →" : "กลับหน้าบทเรียน →"}</button> : <button onClick={() => { setAns({}); setResult(null); }} style={css.btn(B.red, B.white)}>ทำใหม่</button>}</div>}
          </div>
        )}
      </div></div>);
  }
...
function Certificate({ user, go }) {
  ...
  const coupon = load("coupon", null) || genCoupon(); if (!load("coupon", null)) save("coupon", coupon);
  ...
  return (<div style={{ ...css.page, padding: 20 }}><div style={{ maxWidth: 480, margin: "0 auto" }}>
    ...
    <div style={{ background: B.white, borderRadius: 20, padding: 4, boxShadow: "0 8px 32px rgba(0,0,0,.1)" }}><div style={{ border: `3px solid ${B.gold}`, borderRadius: 16, padding: "32px 20px", textAlign: "center", position: "relative", background: "linear-gradient(180deg, #FFFEF7 0%, #FFFFFF 100%)" }}>
      {[{top:8,left:8},{top:8,right:8},{bottom:8,left:8},{bottom:8,right:8}].map((p, i) => (<div key={i} style={{ position: "absolute", ...p, width: 18, height: 18, borderTop: i < 2 ? `2px solid ${B.gold}` : "none", borderBottom: i >= 2 ? `2px solid ${B.gold}` : "none", borderLeft: i % 2 === 0 ? `2px solid ${B.gold}` : "none", borderRight: i % 2 === 1 ? `2px solid ${B.gold}` : "none" }}/>))}
      ...
      <div style={{ fontSize: 10, color: B.dkGray, marginBottom: 16 }}>ศูนย์ฝึกอบรม CPR & AED มาตรฐานสากล</div>
      ...
      <div style={{ fontSize: 20, fontWeight: 300, color: B.dkGray, marginBottom: 3, letterSpacing: 1 }}>ใบประกาศนียบัตร</div>
      ...
      <div style={{ fontSize: 12, color: B.dkGray, lineHeight: 1.6 }}>ผ่านหลักสูตร<br/><strong style={{ fontSize: 13 }}>การช่วยชีวิตขั้นพื้นฐาน CPR & AED ออนไลน์</strong><br/>มาตรฐานสากล</div>
    ...
    <div style={{ background: `${B.red}08`, borderRadius: 16, padding: 20, marginTop: 16, textAlign: "center", border: `1px solid ${B.red}18` }}>
      <div style={{ fontSize: 15, fontWeight: 700, color: B.red, marginBottom: 4 }}>คูปองส่วนลด ฿100 สำหรับคอร์ส On-site!</div>
      <div style={{ fontSize: 13, color: B.dkGray, lineHeight: 1.5, marginBottom: 4 }}>แสดงใบประกาศนียบัตร หรือแจ้งรหัส</div>
      <div style={{ fontSize: 22, fontWeight: 800, color: B.red, letterSpacing: 3, fontFamily: "monospace", marginBottom: 12 }}>{coupon}</div>
      <a href="https://lin.ee/jiacpr" target="_blank" rel="noopener noreferrer" style={{ ...css.btn(B.red, B.white), display: "inline-block", textDecoration: "none" }}>สมัครเรียน On-site →</a>
    </div>
